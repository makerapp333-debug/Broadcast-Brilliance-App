...
Configure when deploying on **DigitalOcean** (and later for app-owned Auth):
...
| Auth callback (future) | `/auth/callback` or `/auth/callback.html` | App auth redirect on DigitalOcean domain when Auth is connected |
